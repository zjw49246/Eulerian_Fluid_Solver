...
def main():
    ...
    result_dir = f"./results_jacobi_{p_jacobi_iters}"
    if method == method_cg:
        result_dir = f"./results_cg_{p_conjugate_gradients_iters}"
    elif method == method_mgpcg:
        result_dir = f"./results_mgpcg_{p_MGPCG_iters}"
    if test_mode:
        result_dir += "_test_mode"
    ...
    while gui.running:
        ...
        if visualize_c:
            ...
            if record_video:
                video_manager.write_frame(dyes_pair.cur)
        elif visualize_v:
